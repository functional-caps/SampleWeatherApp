...
/*
 ...
 We quickly added a separatedBy argument to zeroOrMore, but it can be very useful to parse out an array of values without a separator. Give separatedBy a default parser for this behavior. Is this a parser we’ve already encountered?
 
 */

func zeroOrMoreDef<A>(_ parser: Parser<A>,
                   separatedBy s: Parser<Void> = always(())) -> Parser<[A]> {
    return Parser<[A]> { str in
        var rest = str
        var results = [A]()
        while let result = parser.run(&str) {
...
func zeroOrMoreUntil<A>(
    ...
    return Parser<[A]> { str in
        ...
        while let result = parser.run(&str) {
            rest = str
            results.append(result)
            if s.run(&str) == nil || u.run(&str) != nil {
                return results
            }
        }
        str = rest
...
zeroOrMoreUntil(money, until: literal("#"))
    .run("€42£42#$42,")
    .match

func oneOrMoreUntil<A>(_ parser: Parser<A>,
                  separatedBy: Parser<Void> = always(()),
                  until: Parser<Void>) -> Parser<[A]> {
    return zeroOrMoreUntil(parser, separatedBy: separatedBy, until: until)
        .flatMap { $0.isEmpty ? .never : always($0) }
}

oneOrMoreUntil(money, until: literal("#"))
    .run("€42£42#$42,")
    .match
...
func zeroOrMoreUntilDef<A>(
    _ parser: Parser<A>,
    separatedBy s: Parser<Void> = always(()),
    until u: Parser<Void> = .never
) -> Parser<[A]> {
    return Parser<[A]> { str in
        var rest = str
        var results = [A]()
...
func oneOrMoreUntilDef<A>(_ parser: Parser<A>,
                       separatedBy: Parser<Void> = always(()),
                       until: Parser<Void> = .never) -> Parser<[A]> {
    return zeroOrMoreUntil(parser, separatedBy: separatedBy, until: until)
        .flatMap { $0.isEmpty ? .never : always($0) }
}
...
// I think it should return the first parser that doesn't fail

func oneOf<A>(_ parsers: [Parser<A>]) -> Parser<A> {
    return Parser<A> { str in
        let copyStr = str
        var result: A? = nil
        _ = parsers.first(where: { parser -> Bool in
